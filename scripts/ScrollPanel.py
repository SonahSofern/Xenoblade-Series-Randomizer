...
class ScrollablePanel():
    def __init__(self, parent):
        ...
        self.CreateScrollBars([self.outerFrame], [self.canvas], [self.innerFrame])
        garbList.append(self)
        Theme.CanvasesForStyling.append(self.canvas)

    def CreateScrollBars(self, OuterFrames:list[ttk.Frame], Canvases:list[Canvas], InnerFrames:list[ttk.Frame], genScrollbar = True):
        for i in range(len(Canvases)):
            ...
            OuterFrames[i].bind("<Enter>", lambda e, canvas=Canvases[i]: canvas.bind_all("<MouseWheel>", lambda event: self._on_mousewheel(event, canvas)))
