...
class StatedElement():
    '''Class for elements with enabled and disabled states'''
    @abstractmethod
...
# Add buttons like low medium high max custom that (unless the option is custom) hide the percentages and simplify things visually for users based on the min and max determines the values
class SpinBox(test, StatedElement):
    def __init__(self, parent, min, max, default, description, width, increment, labelStyle):
        self.min = min
        self.max = max
        ...
        self.spinBoxObj.bind("<FocusOut>", lambda: self.EmptyboxHandler())
        self.spinBoxLabel = ttk.Label(parent, text=self.description, anchor="w", style=f"{labelStyle}.TLabel")
        ...
        self.disable_spinbox_scroll()
        
    def validateSpinbox(self, input, min, max):
    ...
    def disable_spinbox_scroll(self):
    ...
    def EmptyboxHandler(self):
        '''Because you can delete the entire string in a box if you defocus it while its empty we set a safe value'''
        try:
            self.spinBoxVal.get() # If we cannot get the value just safely set it to the smalllest value
        except:
    ...
    def SetState(self, state):
        ...
        self.spinBoxLabel.state([state]) 


        
    def getSpinValue():

class Suboption(test, StatedElement):
    pass

# Make an adapter for the way we current call Option class after changing it
def OptionAdapter():# All the normal args
    MainOption()
    SubOption()
    

class MainOption(test, StatedElement):
    def __init__(self, _name:str ="No Name", _tab =1, _desc:str= "No Description", commands:list = [], defState = False, prio = 50, hasSpinBox = False, spinMin = 0, spinMax = 100, spinDesc = "% randomized", spinWidth = 3, spinIncr = 10, spinDefault = 100, descData = None, preRandoCommands:list = [], isDevOption = False, stepSpeed = 0.05, filePlaceCommands:list = []):
        ...
        self.clickCommands = []
        self.statedElements:list[StatedElement] = []
    ...
    def GenStandardOption(self, parentTab, style):    # This probably shouldnt be a class function what if we want to make a nonstandard option we could make a carveout and let you call a custom function but how would you set everything with a custom function
        ...
        # % Boxes
        if self.hasSpinBox:
            SpinBox(optionPanel , self.spinBoxMin, self.spinBoxMax, self.spinDefault, self.spinDesc, self.spinWidth, self.spinIncr)
    ...
    def StateUpdate(self):
        '''When the main option is clicked on it updates the states'''
        for element in self.statedElements:
            element.SetState(self.GetState())
...
class SubOption():
    def __init__(self, _name, _parent:MainOption, commands = [], defState = True, prio = 0, spinDefault = 0, spinMin = 0, spinMax = 100, spinWidth = 3, spinIncr = 10, hasSpinBox = False, spinPadX = 15, spinDesc = "", preRandoCommands:list = []):
        self.name = _name
        self.checkBoxVal = BooleanVar
...
class MutuallyExclusivePairing():
    ...
    def __init__(self, group1:list[MainOption], group2:list[MainOption]):
        for op in group1:
...
def MutuallyExclusiveToggle(op:MainOption, pairGroup:list[MainOption]):
...
def AskToChooseOption(enabledOption, conflictingOptions:list[MainOption]):
