import json, random
def RandomizeFieldSkills(optionsDict): # Make logic to have all skills in the game
    # Drivers
    with open("./_internal/JsonOutputs/common/CHR_Bl.json", 'r+', encoding='utf-8') as bladeFile:
        ...
        KeepQuestSkills = optionsDict["Blade Field Skills"]["subOptionObjects"]["Keep Quest Skills"]["subOptionTypeVal"].get()
        QuestSkillPool = [74, 73, 31, 35, 46, 33,37,39,26,43,44,57,34,40,51,29,30,45,47,27,36,28,32,42,50,48,58,41,49]     


        # print(list((x for x in BaseSkillPool if x not in QuestSkillPool)))

        # Carveouts to make sure the story is completeable
        KeepVanilla = {
            1001: ["FSkill1","FSkill2", "FSkill3"], # Pyra All
            1008: ["FSkill2"] # Roc Miasma
        }
        Slots = ["FSkill1","FSkill2", "FSkill3"]
        
        if not KeepQuestSkills: # Add in our quest skills
        ...
        for blade in bladeData["rows"]:     
            ...
            for slot in Slots:
                ...
                if KeepQuestSkills and (blade[slot] in QuestSkillPool): # Keep unique skills on that blade
                    continue
