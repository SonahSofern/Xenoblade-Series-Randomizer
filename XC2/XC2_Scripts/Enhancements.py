...
TitanExecute = Enhancement("Titan KO",22, 229, [6], Baby)
MachineExecute = Enhancement("Machine KO",22, 228, [5], Baby)
HumanoidExecute = Enhancement("Humanoid KO",22, 227, [4], Baby)
AquaticExecute = Enhancement("Aquatic KO",22, 226, [3], Baby)
AerialExecute = Enhancement("Aerial KO",22, 225, [2], Baby)
InsectExecute = Enhancement("Insect KO",22, 224, [1], Baby)
BeastExecute = Enhancement("Beast KO",22, 223, [0], Baby)
BladeComboDamUp = Enhancement("Bld Combo",23,21, Large)
FusionComboDamUp = Enhancement("Fsn Combo",24,22, Large)
...
ArtDamageHeal = Enhancement("Art Vamp",28, 28, Small) # This goes on arts only or else it will heal from special and arts
...
ToppleDamageUp = Enhancement("Topple Dmg",37,38, Large, DisTag="Topple Damage ↑")
LaunchDamageUp = Enhancement("Launch Dmg",38,39, Large, DisTag="Launch Damage ↑")
SmashDamageUp = Enhancement("Smash Dmg",39,40, Mega, DisTag="Smash Damage ↑")
...
BreakDurationUp = Enhancement("Break Time",47,48, Medium, DisTag="Break Duration ↑")
ToppleDurationUp = Enhancement("Topple Time",48,49, Medium, DisTag="Topple Duration ↑")
LaunchDurationUp = Enhancement("Launch Time",49,50, Medium, DisTag="Launch Duration ↑")
...
BlowdownResistUp = Enhancement("Grounded",73, 71, Medium)
KnockbackResistUp = Enhancement("Steady Stance",74, 72, Medium)
DefenseAnnulResistUp = Enhancement("Unpierceable",75, 73, Medium)
AutoAttackAggroDown = Enhancement("AA Aggro ↓",77, 75, Large)
AutoAttackAggroUp = Enhancement("AA Aggro ↑",78, 76, Large)
SpecialAndArtsAggroDown = Enhancement("Blade Aggro ↓",79, 77, Medium)
SpecialAggroDown = Enhancement("Sp Aggro ↓",79, 79, Medium) # For Specials only 
SpecialAndArtsAggroUp = Enhancement("Blade Aggro ↑",80, 81, Medium)
...
RevivalHP = Enhancement("Revival HP ↑",84, 96, Large, DisTag="Revival HP↑")
RevivalHPTeammate = Enhancement("Helping Hand",85, 97, Large, DisTag="Revival HP↑")
...
PartyGaugeExcellentFill = Enhancement("Excellent Party",101, 109, Small, Description="Fills the Party Gauge on an\n\"Excellent\" Special by [ML:Enhance kind=Param1 ].")
PartyGaugeCritFill = Enhancement("Critical Party",102, 112, Mini, Description="Fills the Party Gauge for\neach critical hit delivered by [ML:Enhance kind=Param1].")
PartyGaugeDriverArtFill = Enhancement("Driver Party",103, 115, Baby, Description="Fills the Party Gauge\nfor each Driver Art used by [ML:Enhance kind=Param1].")
...
PreventAffinityLossOnDeath = Enhancement("Keep Affinity",121, 131)
AffinityUpButtonChallenge = Enhancement("Button Aff ↑",122, 132, Medium)
MissAffinityUp = Enhancement("Miss Aff ↑",123, 133, Small)
DamageTakenAffinityUp = Enhancement("Tank Aff ↑",124,134, Small)
...
HPRestoreFusionCombo = Enhancement("Fusion Heal",161, 172, Baby)
...
RestoreHitDamageToParty = Enhancement("Party Vamp",192, 202, Baby)
...
AegisParty = Enhancement("Party Dream",196, 206, addToList=False)
...
MaxAffinityHeal = Enhancement("Max Aff Heal",141,237,Mini)
AbsorbFireBlock = Enhancement("Null Fire",60, 238, [1], addToList=False)
AbsorbWaterBlock = Enhancement("Null Water",60, 239, [2], addToList=False)
AbsorbWindBlock = Enhancement("Null Wind",60, 240, [3], addToList=False)
AbsorbEarthBlock = Enhancement("Null Earth",60, 241, [4], addToList=False)
AbsorbElectricBlock = Enhancement("Null Electric",60, 242, [5], addToList=False)
AbsorbIceBlock = Enhancement("Null Ice",60, 243, [6], addToList=False)
AbsorbLightBlock = Enhancement("Null Light",60, 244, [7], addToList=False)
AbsorbDarkBlock = Enhancement("Null Dark",60, 245, [8], addToList=False)
ReflectFireBlock = Enhancement("Bounce Fire",61, 246, [1], addToList=False)
ReflectWaterBlock = Enhancement("Bounce Water",61, 247, [2], addToList=False)
ReflectWindBlock = Enhancement("Bounce Wind",61, 248, [3], addToList=False)
ReflectEarthBlock = Enhancement("Bounce Earth",61, 249, [4], addToList=False)
ReflectElectricBlock = Enhancement("Bounce Electric",61, 250, [5], addToList=False)
ReflectIceBlock = Enhancement("Bounce Ice",61, 251, [6], addToList=False)
ReflectLightBlock = Enhancement("Bounce Light",61, 252, [7], addToList=False)
ReflectDarkBlock = Enhancement("Bounce Dark",61, 253, [8], addToList=False)
...
DriverShackRes = Enhancement("Free Driver",218, 281, Medium)
BladeShackRes = Enhancement("Free Blade",219, 282, Medium)
BurstDestroyAnotherOrb = Enhancement("Orb Chain",226, 283, DisTag="Orb Chain")
...
TargetNearbyOrbsChainAttack = Enhancement("Orb Splash",229, 286, Medium, DisTag="Orb Splash")
TargetDamagedNonOpposingElement = Enhancement("Orb Focus",230, 287, DisTag="Orb Focus")
