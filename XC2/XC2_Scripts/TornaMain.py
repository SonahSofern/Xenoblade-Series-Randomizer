...
# hints
# spoiler log
# spoiler log basics are down, I just need to add a section that denotes the logical playthrough of a seed
# object color matches contents. make the name of a check container yellow if it has progression, green if it has helpful items, and normal if it doesn't?
...
def CharacterUnlocks(): # sets up the character unlock keys
    # FLD_QuestTaskIra row 4 should have 3 additional Tasks, of type Collect (3), that mention each individual extra key, AddamKey, MythraKey, HazeKey
    # FLD_QuestTaskIra row 11 should have 3 additional Tasks, of type Collect (3), that mention each individual extra key, HugoKey, BrighidKey, AegaeonKey
...
def CreateSpoilerLog():
    ...
    for option in OptionList:
        ...
        if OptionVal == True:
            for subOption in option.subOptions: # looping through all suboptions
                ...
                if SubOptionVal == True: # if the suboption is a checkbox and checked
    ...
    alllines.append("\n")
    alllines.append(LocTypetoSpoilerLogHeader[AllLocations[0].type])
    alllines.append("\n\n")
    for loc in range(len(AllLocations)):
        ...
        if loc != 0 and CurLocType != LocTypetoSpoilerLogHeader[AllLocations[loc - 1].type]:
            alllines.append("\n")
            alllines.append(LocTypetoSpoilerLogHeader[AllLocations[loc].type])
            alllines.append("\n")
            alllines.append("\n")
