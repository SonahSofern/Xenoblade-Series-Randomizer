...
def RandomizeFLDQuestReward(shopIDs, valTable:Values.ValueTable):
    ...
    with open("XC2/JsonOutputs/common/FLD_QuestReward.json", 'r+', encoding='utf-8') as qstFile:
        ...
        for qst in qstData["rows"]:
            ...
            for i in range(1,5):
                valTable.SelectValuedMember(qst, f"ItemID{i}", IDs.PreciousItems + IDs.TornaPreciousIDs + IDs.InfoIDs + IDs.TornaInfoIDs + IDs.AllCoreCrystals)
...
def GetTreasureBoxValue(tbox, valTable:Values.ValueTable):
    ...
    for i in range(1,9):
        ...
        item:Values.ValuedItem = valTable.GetByIDCat(itemID)
        if item:
