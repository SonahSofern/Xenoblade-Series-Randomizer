...
def Enemies():
    MechonFamily = 1
    TelethiaFamily = 9
    BadForMechon = [30, 31, 32,33] # List of ids for the early game to make sure mechon arent placed here
            ...
            try:   
                with open(f"./XCDE/_internal/JsonOutputs/bdat_ma{file}/BTL_enelist{file}.json", 'r+', encoding='utf-8') as eneAreaFile:
                    ...
                    for enemy in eneAreaData["rows"]:   
                        ...
                        # Mechon Early
                        while (enemy["$id"] in BadForMechon) and (chosen.eneListArea["family"] == MechonFamily):
...
def RingRemoval():
    ...
    with open(f"./XCDE/_internal/JsonOutputs/bdat_ma1401/FieldLock1401.json", 'r+', encoding='utf-8') as lockFile:
        ...
        lockFile.seek(0)
        lockFile.truncate()
        json.dump(lockData, lockFile, indent=2, ensure_ascii=False)