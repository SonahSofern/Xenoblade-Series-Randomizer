import json, IDs, random, Options

def MusicRando():
    with open("./XCDE/_internal/JsonOutputs/bdat_common/bgmlist.json", 'r+', encoding='utf-8') as bgmFile:
        bgmData = json.load(bgmFile)
        curThemes = EnvironmentThemes + CutsceneThemes
        allBattleThemes = BattleThemes + BadBattleThemes
        curBattleThemes = BattleThemes
        if not Options.MusicOption_GoodBattleThemes.GetState():
            curBattleThemes.extend(BadBattleThemes)
        for bgm in bgmData["rows"]:
            if bgm["file_name"] in (allBattleThemes):
                bgm["file_name"] = random.choice(curBattleThemes)
            elif bgm["file_name"] in (Jingles):
                bgm["file_name"] = random.choice(Jingles)
            else:
                bgm["file_name"] = random.choice(curThemes)
