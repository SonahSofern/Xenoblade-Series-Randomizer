...
# General
ShopOption = MainOption("Shops", General, "Randomizes the contents of shops", [lambda: Items.Shops()], descData=lambda: Items.ShopsDesc()) # Key item rando settings would be fun 
TradeOption = MainOption("Trading", General, "Randomizes the offers of NPC trades", [lambda: Items.TradeOptions()], descData=lambda: Items.TradeOptionsDesc())

EnemyDropOption = MainOption("Enemy Drops", General, "Randomizes the chosen types of loot from enemies", [lambda: Items.EnemyDrops()], descData=lambda: Items.EnemyDropsDesc())
...
CollectableOptions = MainOption("Collectable Orbs", General, "Randomizes collectables on the field into the chosen types", [lambda: Items.Collectables()], descData=lambda: Items.CollectDesc())
...
CollectapediaOptions = MainOption("Collectapedia Rewards", General, "Randomizes rewards from the collectapedia into the chosen types", [lambda: Items.Collectapedia()], descData=lambda: Items.CollectapediaDesc())
...
GiantsChestOption = MainOption("Giants Chests", General, "Randomizes the contents of Giants Chests into the chosen types", [lambda: Items.GiantsChests()], descData=lambda: Items.GiantsChestsDesc())
...
QuestRewardsOption = MainOption("Quest Rewards", General, "Randomizes the rewards from quests into the chosen types", [lambda: Items.QuestRewards()], descData=lambda: Items.QuestRewardsDesc())
...
# Enemy
NormalEnemyOption = MainOption("Normal Monsters", Enemies, "Randomizes normal monsters into the chosen types", [lambda: EnemiesScript.Enemies(IDs.NormalEnemies, NormalEnemyOption, NormalEnemyOption_Normal, NormalEnemyOption_Unique, NormalEnemyOption_Boss, NormalEnemyOption_Superboss, NormalEnemyOption_Size.GetState())], descData=lambda: EnemiesScript.EnemyDesc(NormalEnemyOption.name), hasSpinBox = True)
...
UniqueEnemyOption = MainOption("Unique Monsters", Enemies, "Randomizes unique monsters, including superbosses, into the chosen types", [lambda: EnemiesScript.Enemies(IDs.UniqueEnemies + IDs.SuperbossEnemies, UniqueEnemyOption, UniqueEnemyOption_Normal, UniqueEnemyOption_Unique, UniqueEnemyOption_Boss, UniqueEnemyOption_Superboss, UniqueEnemyOption_Size.GetState())], descData=lambda: EnemiesScript.EnemyDesc(UniqueEnemyOption.name), hasSpinBox = True)
...
# Character
GemOption = MainOption("Gems", Character, "Randomizes the effects of Gems and Crystals", [lambda: Gems.Gems()], descData=lambda: Gems.GemDescriptions())
...
AffinityTreeOption = MainOption("Skill Trees", Character, "Randomizes all character's skill trees", [lambda: SkillTrees.SkillRando()], descData=lambda: SkillTrees.SkillTreeDesc(), hasSpinBox = True)
...
PlayerArtsOption = MainOption("Arts", Character, "Randomizes character's arts and their effects", [lambda: PcArts.RandomizePcArts()], descData=lambda: PcArts.ArtsDescriptions())
...
EquipmentOption = MainOption("Armor", Character, "Randomizes effects of Armor", [lambda: Armor.ArmorRando()], descData=lambda: Armor.ArmorDesc())
...
WeaponOption = MainOption("Weapons", Character, "Randomizes effects of Weapons", [lambda: Weapons.WeaponRando()], descData=lambda: Weapons.WepDesc())
...
# Misc
BattleMusicOption = MainOption("Battle Music", Musica, "Randomizes the chosen battle themes onto all battle themes", [lambda: Music.MusicRando(Music.AllBattleThemes, Music.UsedBattleThemes)]) #https://xenobladedata.github.io/xb1de/bdat/bdat_common/bgmlist.html
for song in Music.AllBattleThemes:
    song.CreateOption(BattleMusicOption, Music.UsedBattleThemes)
BossMusicOption = MainOption("Boss Music", Musica, "Randomizes the chosen boss themes onto all boss themes", [lambda: Music.MusicRando(Music.AllBossThemes, Music.UsedBossThemes)])
for song in Music.AllBossThemes:
    song.CreateOption(BossMusicOption, Music.UsedBossThemes)
EnvironmentCutsceneMusicOption = MainOption("Environment/Cutscene Music", Musica, "Randomizes the chosen environment/cutscene themes onto all environment themes", [lambda: Music.MusicRando(Music.AllEnvironmentThemes + Music.AllCutsceneThemes, Music.UsedEnvironmentThemes + Music.UsedCutsceneThemes)])
...
JingleMusicOption = MainOption("Jingles", Musica, "Randomizes the chosen jingles onto all jingles", [lambda: Music.MusicRando(Music.AllJingles, Music.UsedJingles)])
for song in Music.AllJingles:
    song.CreateOption(JingleMusicOption, Music.UsedJingles)

# QOL
TutorialSkipsOption = MainOption("Tutorial Skips", QOL, "Skips tutorial popups", filePlaceCommands=[lambda: XCRandomizer.FilePlacer(["Loader/plugins/xcdeRemoveTutorials.nro"], "../skyline/plugins", game=game)])
FasterLvOption = MainOption("EXP Boost", QOL, "Decreases level up requirements by a set amount (Recommended 3x to rush the story)", [lambda: Helper.MathmaticalColumnAdjust(["./XCDE/JsonOutputs/bdat_common/BTL_growlist.json"], ["level_exp"], [f'row[key] // {FasterLvOption.GetSpinbox()}'])], hasSpinBox = True, spinMin = 0, spinMax = 256, spinIncr = 1, spinDesc = "x", spinDefault=3)
FasterSkillTrees = MainOption("SP Boost", QOL, "Decreases SP (skill point) requirements for skill trees", [lambda: Helper.MathmaticalColumnAdjust(["./XCDE/JsonOutputs/bdat_common/BTL_PSVskill.json"], ["point_PP"], [f'row[key] // {FasterLvOption.GetSpinbox()}'])], hasSpinBox = True, spinMin = 0, spinMax = 256, spinIncr = 1, spinDesc = "x", spinDefault=2)
FasterArtLevels = MainOption("AP Boost", QOL, "Increases AP (art point) gains for art level ups",[lambda: Helper.MathmaticalColumnAdjust(["./XCDE/JsonOutputs/bdat_common/BTL_growlist.json"], ["en_ap"], [f'row[key] * {FasterLvOption.GetSpinbox()}'])], hasSpinBox = True, spinMin = 0, spinMax = 256, spinIncr = 1, spinDesc = "x", spinDefault=2)
MovespeedOption = MainOption("Quickstep", QOL, "The gem man will gift you two free quickstep gems", [lambda: MiscQOL.Quickstep()], hasSpinBox=True, spinDesc="% Speed", spinMax=100)
AreaAffinityOption = MainOption("Easy Affinity", QOL, "Area affinity is maxed out after any quest", [lambda: MiscQOL.QuestAffinity()])
...
NPCScaleOption = MainOption("NPC Scale", Funny, "Randomizes a % of npc sizes.", [lambda: Scales.NPCScales()], hasSpinBox = True)
RemoveStartingArmorOption = MainOption("Remove Starting Equipment", Funny, "Removes starting armor on all the main characters.", [lambda: Armor.RemoveStartingGear()])
NPCModelsOption = MainOption("NPC Models", Funny, "Randomizes NPC models *Experimental, can cause crashes", [lambda: NPC.NPCModelRando()], hasSpinBox = True)
