...
fn skipTutorialHook(_this_ptr: *mut c_void, _short: c_ushort, _bool: bool) -> bool{
    unsafe {
        if (!call_original!(_this_ptr, _short, _bool)){ // call getTutorial and if one exists then we call setTutorial immediately
