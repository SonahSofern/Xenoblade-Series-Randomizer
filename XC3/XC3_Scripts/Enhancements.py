...
class Enhancement:
    ...
    def CreateEffect(self, BTL_EnhanceData, overrideParam1 = None, overrideParam2 = None):
...
def CreateEnhancements():                 
    ...
    Enhancement("Diurnal", 95, 120, 1, A, [], skillIcon=0) # Night Vision
    Enhancement("Nocturnal", 96, 121, 1, M, [], skillIcon=0) # Sunlight Eye
    ...
    Enhancement("", 114, 136, 1, M, [], [], skillIcon=0) # <C4D784E6>
    Enhancement("", 115, 138, 1, M, [], [], skillIcon=0) # Accuracy/Evasion Up: Allies
    # Enhancement("", 116, 0, 1, M, [], [], skillIcon=0) # <554E9057>
    Enhancement("", 117, 139, 1, M, [], skillIcon=0) # Slower Attack Aggro
    Enhancement("", 118, 140, 1, M, [], skillIcon=0) # All Defenses Plus
    Enhancement("", 119, 141, 1, M, [], skillIcon=0) # Perfect Hit/Evasion
    Enhancement("", 120, 142, 1, M, [], skillIcon=0) # Reflection
    Enhancement("", 121, 143, 1, M, [], skillIcon=0) # Toppled/Launched↑
    # Enhancement("", 122, 0, 1, M, [], skillIcon=0) # <1902ADDB>
    Enhancement("", 123, 144, 1, M, [], skillIcon=0) # Enemy KO Heal: Allies
    # Enhancement("", 124, 0, 1, M, [], [], skillIcon=0) # <E48EE6DF>
    Enhancement("", 125, 145, 1, M, [], [], skillIcon=0) # Power Up (Dire Battle)
    # Enhancement("", 126, 0, 1, M, [], skillIcon=0) # <2B78D893>
    Enhancement("", 127, 149, 1, M, [], skillIcon=0) # Resist Combo Reaction
    Enhancement("", 128, 150, 1, M, [], skillIcon=0) # Unique/Boss↑
    Enhancement("", 129, 151, 1, M, [], skillIcon=0) # Continuous Arts
    Enhancement("", 130, 152, 1, M, [], skillIcon=0) # Piercing Attacks
    Enhancement("", 131, 154, 1, M, [], skillIcon=0) # Auto-Attack Speed Up
    Enhancement("", 132, 156, 1, M, [], skillIcon=0) # <34426628>
    Enhancement("", 133, 159, 1, M, [], skillIcon=0) # <BB6A576F>
    Enhancement("", 134, 160, 1, M, [], skillIcon=0) # Recover Recharge (Critical)
    Enhancement("", 135, 161, 1, M, [], [], skillIcon=0) # Regeneration (Low HP)
    Enhancement("", 136, 162, 1, M, [], [], skillIcon=0) # Damage Up (Talent Art)
    Enhancement("", 137, 163, 1, M, [], [], skillIcon=0) # Damage Counter: Reaction
    # Enhancement("", 138, 0, 1, M, skillIcon=0) # <8220967D>
    Enhancement("", 139, 164, 1, M, [], skillIcon=0) # Bigger Cancel Window
    Enhancement("", 140, 165, 1, M, [], skillIcon=0) # <3F363F42>
    Enhancement("", 141, 167, 1, M, [], skillIcon=0) # <0C909AAE>
    Enhancement("", 142, 168, 1, M, [], skillIcon=0) # Ranged Evasion
    Enhancement("", 143, 169, 1, M, [], skillIcon=0) # Hold Aggro (Self KO)
    Enhancement("", 144, 170, 1, M, [], skillIcon=0) # Damage Up (Ally KO)
    # Enhancement("", 145, 0, 1, M, [], [], skillIcon=0) # <4724B11C>
    Enhancement("", 146, 171, 1, M, [], [], skillIcon=0) # <1DDE9148>
    Enhancement("", 147, 172, 1, M, [], [], skillIcon=0) # Attack Shift
    Enhancement("", 148, 173, 1, M, [], [], skillIcon=0) # Defense Shift
    Enhancement("", 149, 174, 1, M, [], skillIcon=0) # Debuff Resist: All
    Enhancement("", 150, 175, 1, M, [], skillIcon=0) # Shorter Debuff Timers
    Enhancement("", 151, 176, 1, M, [], skillIcon=0) # Damage Up (Cancel)
    # Enhancement("", 152, 0, 1, M, [], skillIcon=0) # <CE6DB1B1>
    Enhancement("", 153, 177, 1, M, skillIcon=0) # Soul Hacking
    Enhancement("", 154, 178, 1, M, [], skillIcon=0) # <1DD0CB32>
    ...
    Enhancement("", 158, 179, 1, M, [], skillIcon=0) # <C719FE0D>
    Enhancement("", 159, 180, 1, M, [], skillIcon=0) # <1C89FC37>
    Enhancement("", 160, 181, 1, M, [], skillIcon=0) # <621F1009>
    Enhancement("", 161, 182, 1, M, [], skillIcon=0) # Damage Up (Enemy Arts)
    Enhancement("", 162, 183, 1, M, [], skillIcon=0) # <3C2BDC90>
    Enhancement("", 163, 184, 1, M, [], skillIcon=0) # Accuracy Up (Enemy Arts)
    Enhancement("", 164, 185, 1, M, [], skillIcon=0) # <0282C4D5>
    Enhancement("", 165, 186, 1, M, [], [], skillIcon=0) # Wounded Enemy↑
    Enhancement("", 166, 189, 1, M, [], skillIcon=0) # Accuracy Up (Miss)
    Enhancement("", 167, 190, 1, M, [], [], skillIcon=0) # Debuffed Enemy↑
    # Enhancement("", 168, 0, 1, M, [], skillIcon=0) # <ED6CBA7D>
    Enhancement("", 169, 193, 1, M, [], skillIcon=0) # <3F91CECC>
    Enhancement("", 170, 194, 1, M, [], skillIcon=0) # <E7C12D95>
    # Enhancement("", 171, 0, 1, M, [], skillIcon=0) # <C29024C3>
    Enhancement("", 172, 197, 1, M, [], [], skillIcon=0) # Damage Bomb (Self KO)
    Enhancement("", 173, 198, 1, M, [], skillIcon=0) # Party Recharge (Self KO)
    # Enhancement("", 174, 0, 1, M, [], [], skillIcon=0) # <48F464E4>
    Enhancement("", 175, 199, 1, M, skillIcon=0) # Perfect Hit (Cancel)
    # Enhancement("", 176, 0, 1, M, [], [], skillIcon=0) # <A8B4FF50>
    Enhancement("", 177, 200, 1, M, [], skillIcon=0) # Fortitude (Art Duration)
    Enhancement("", 178, 202, 1, M, [], skillIcon=0) # Damage Up (Full Recharge)
    Enhancement("", 179, 203, 1, M, [], skillIcon=0) # <0007E213>
    # Enhancement("", 180, 0, 1, M, [], [], skillIcon=0) # Persistent Damage Up
    Enhancement("", 181, 206, 1, M, [], [], skillIcon=0) # Sequential Auto-Attack Up
    Enhancement("", 182, 210, 1, M, [], skillIcon=0) # Stance Duration Up
    Enhancement("", 183, 211, 1, M, [], skillIcon=0) # Buff Heal
    # Enhancement("", 184, 0, 1, M, [], [], skillIcon=0) # <CF9BFB20>
    Enhancement("", 185, 215, 1, M, [], skillIcon=0) # Tender Auto-Attack
    Enhancement("", 186, 216, 1, M, [], skillIcon=0) # Tender Arts
    Enhancement("", 187, 217, 1, M, [], skillIcon=0) # Fusion Arts Up
    Enhancement("", 188, 218, 1, M, [], [], skillIcon=0) # <27C116CB>
    Enhancement("", 189, 219, 1, M, [], skillIcon=0) # Aggro Reduce Up (Targeted)
    Enhancement("", 190, 220, 1, M, [], skillIcon=0) # Draw Aggro (Evasion)
    Enhancement("", 191, 223, 1, M, [], skillIcon=0) # Incremental Damage Up
    # Enhancement("", 192, 0, 1, M, [], [], skillIcon=0) # Preemptive Buff
    Enhancement("", 193, 225, 1, M, skillIcon=0) # <5EA32440>
    Enhancement("", 194, 226, 1, M, [], skillIcon=0) # Damage Up (Buff No.)
    Enhancement("", 195, 227, 1, M, [], [], skillIcon=0) # Aerial Recovery
    Enhancement("", 196, 229, 1, M, [], skillIcon=0) # Risky Recharge Up
    Enhancement("", 197, 230, 1, M, [], skillIcon=0) # Damage Up (Elem. Discharge)
    Enhancement("", 198, 231, 1, M, [], [], skillIcon=0) # EX Damage (Base Attack)
    Enhancement("", 199, 232, 1, M, [], [], skillIcon=0) # EX Damage (Base Healing)
    Enhancement("", 200, 233, 1, M, [], [], skillIcon=0) # <59C16213>
    # Enhancement("", 201, 0, 1, M, [], [], skillIcon=0) # <3BFA4DC7>
    # Enhancement("", 202, 0, 1, M, [], skillIcon=0) # Damage Up (Ally Heal)
    Enhancement("", 203, 235, 1, M, [], [], skillIcon=0) # Prevent Attack: Nearby Allies       
    Enhancement("", 204, 236, 1, M, [], skillIcon=0) # Field Duration Up
    Enhancement("", 205, 237, 1, M, [], skillIcon=0) # Field Damage Up
    Enhancement("", 206, 238, 1, M, [], skillIcon=0) # Field Dissolution Dmg. Up
    Enhancement("", 207, 239, 1, M, [], skillIcon=0) # <3C8F6C0E>
    Enhancement("", 208, 241, 1, M, skillIcon=0) # Enable Rescue
    Enhancement("", 209, 242, 1, M, [], skillIcon=0) # Evasion Chance
    Enhancement("", 210, 243, 1, M, [], skillIcon=0) # Physical Damage Up
    Enhancement("", 211, 244, 1, M, [], skillIcon=0) # Ether Damage Up
    Enhancement("", 212, 245, 1, M, [], skillIcon=0) # Reaction Success Up
    Enhancement("", 213, 246, 1, M, [], skillIcon=0) # Talent Art Starter
    Enhancement("", 214, 248, 1, M, [], skillIcon=0) # Guard Efficiency Up
    # Enhancement("", 215, 0, 1, M, [], skillIcon=0) # <6B7B5B21>
    Enhancement("", 216, 249, 1, M, [], [], skillIcon=0) # Combo HP Heal: Allies
    Enhancement("", 217, 250, 1, M, [], skillIcon=0) # Pierce Physical Defense
    Enhancement("", 218, 251, 1, M, [], skillIcon=0) # Pierce Ether Defense
    Enhancement("", 219, 252, 1, M, [], skillIcon=0) # Critical Rate Up (Hit)
    Enhancement("", 220, 253, 1, M, [], skillIcon=0) # Healing Up (Debuff Clear)
    Enhancement("", 221, 254, 1, M, [], skillIcon=0) # Damage Up (Buff)
    Enhancement("", 222, 255, 1, M, [], skillIcon=0) # Damage Up (Debuff)
