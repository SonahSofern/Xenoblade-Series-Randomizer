...
UniqueEnemyOption = Option("Unique Monsters", Enemies, "Randomizes unique monsters, including superbosses, into the chosen types", [lambda: Enemy.Enemies(IDs.UniqueMonsters + IDs.SuperbossMonsters, UniqueEnemyOption_Normal, UniqueEnemyOption_Unique, UniqueEnemyOption_Boss, UniqueEnemyOption_Superboss, UniqueEnemyOption, True, False)], descData=lambda: Enemy.EnemyDesc(UniqueEnemyOption.name), hasSpinBox = True)
...
FasterAffinityGrowthOption = Option("AP Boost", QOL, "Decreases AP required for each Affinity Growth node (Future Redeemed)", [lambda: Quality.AffinityGrowthReduction(FasterAffinityGrowthOption.GetSpinbox())], hasSpinBox=True, spinDefault=2, spinIncr = 1, spinDesc = "x Faster")
