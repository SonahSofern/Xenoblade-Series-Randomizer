from scripts.Interactables import MainOption, SubOption, Label
...
ShopOption = MainOption("Shops", General, "Randomizes shop contents", [lambda: Items.Shops()])
...
EnemyNormalDropOption = MainOption("Enemy Drops", General, "Randomizes Enemy Drops (not including materials)", [lambda: Items.EnemyDrops()])
...
TreasureBoxOption = MainOption("Containers", General, "Randomizes the contents of Containers and Supply Drops", [lambda: Items.TreasureBoxes()])
...
QuestRewardsOption = MainOption("Quest Rewards", General, "Randomizes the item rewards from Quests", [lambda: Items.QuestRewards()])
...
CollectapediaRewardsOption = MainOption("Collectopaedia Card Rewards", General, "Randomizes the item rewards from Collectopaedia Cards", [lambda: Items.CollectopaediaCards()])
...
UniqueEnemyOption = MainOption("Unique Monsters", Enemies, "Randomizes unique monsters, including superbosses, into the chosen types", [lambda: Enemy.Enemies(IDs.UniqueMonsters + IDs.SuperbossMonsters, UniqueEnemyOption_Normal, UniqueEnemyOption_Unique, UniqueEnemyOption_Boss, UniqueEnemyOption_Superboss, UniqueEnemyOption, UniqueEnemyOption_MatchSize.GetState(), False)], descData=lambda: Enemy.EnemyDesc(UniqueEnemyOption.name), hasSpinBox = True)
...
AccessoriesOption = MainOption("Accessories", Character, "Randomizes the effects of Accessories", [lambda: Accessories.AccessoryRando()])
GemsOption = MainOption("Gems", Character, "Randomizes the effects of Gems", [lambda: Gems.GemRando()])

MajorSkillOption = MainOption("Major Skills", Character, "Randomizes the chosen major skill categories", [lambda: Skills.SkillRandoMain()], hasSpinBox=True)
...
MinorSkillOption = MainOption("Minor Skills", Character, "Randomizes minor skills among themselves", [lambda: Skills.MinorSkillShuffle(IDs.InoTreeNodes + IDs.UroTreeNodes + IDs.DLC4TreeNodes)])
...
class ArtShuffleOption():
    def __init__(self, name, targetIDs, extraIgnoreKeys = []):
        ...
        mainOption = MainOption(name, Character, "Randomizes " + name + " into the chosen types", [lambda: Arts.ArtRando(targetIDs, subClassArts, subOuroArts, subTalentArts, subOuroTalentArts, subHackerArts, mainOption.GetSpinbox(), extraIgnoreKeys)], hasSpinBox=True)
...
CostumesOption = MainOption("Class Costumes", Character, "Randomizes class outfits", [lambda: Costumes.LearnedClassOutfits()])
...
TutorialSkipOption = MainOption("Tutorial Skips", QOL, "Removes tutorials, as a side effect also gives access to all systems from the start", [lambda: Shortcuts.TutorialSkips()])
FasterLevelsOption = MainOption("EXP Boost", QOL, "Decreases EXP required for each levelup", [lambda: Helper.MathmaticalColumnAdjust(["XC3/JsonOutputs/btl/BTL_Grow.json"], ["LevelExp", "LevelExp2"], [f'row[key] // {FasterLevelsOption.GetSpinbox()}'])], hasSpinBox=True, spinDefault=2, spinIncr = 1, spinDesc = "x Faster")
FasterApitudeOption = MainOption("CP Boost", QOL, "Increases the rate at which your characters learn classes", [lambda: Quality.ClassAptitude()], hasSpinBox=True, spinDesc = "x Faster", spinDefault=2)
MoveSpeedOption = MainOption("Speed Boost", QOL, "Colony 4's affinity reward will be instant and a movespeed deed", [lambda: Quality.EarlyMoveSpeed()], hasSpinBox=True, spinDesc = "% Speed", spinMin=1, spinMax=255, spinIncr=10)
...
EarlyArtsCancelOption = MainOption("Early Arts Cancel", QOL, "The Art of Flow is given during the introduction", [lambda: Quality.ArtOfFlowEarly()])
GemCraftingOption = MainOption("Easy Gem Crafting", QOL, "Reduces the material requirements for gem crafting", [lambda: Gems.EasyGemCrafting()])
